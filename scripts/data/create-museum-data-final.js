...
async function insertMuseumData() {
  ...
  try {
    ...
    console.log(`使用用户ID: ${adminUserId}\n`);
    ...
    const statsQuery = `
      SELECT 
        (SELECT COUNT(*) FROM letters WHERE visibility = 'public') as public_letters,
        (SELECT COUNT(*) FROM museum_items WHERE source_type = 'letter' AND status = 'approved') as museum_items,
        (SELECT COUNT(*) FROM museum_exhibitions WHERE status = 'active') as active_exhibitions
    `;
    
    const stats = await client.query(statsQuery);
    console.log('\n📊 数据统计:');
    console.log(`   公开信件数量: ${stats.rows[0].public_letters}`);
    console.log(`   博物馆物品: ${stats.rows[0].museum_items}`);
    console.log(`   活跃展览: ${stats.rows[0].active_exhibitions}`);
    
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('\n❌ 插入数据时发生错误:', error.message);
