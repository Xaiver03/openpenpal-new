...
# Manual test for OpenPenPal startup modes
# This script tests each startup mode individually without timeout command
...
# Test report file
REPORT_FILE="$LOG_DIR/startup-test-manual-$(date +%Y%m%d-%H%M%S).md"
...
check_service_health() {
    ...
    for path in "/health" "/" ""; do
        if curl -s -f -o /dev/null --max-time 3 "$url$path" 2>/dev/null; then
            return 0
        fi
    done
    return 1
...
# Function to wait for startup with manual timeout
wait_for_startup() {
    local mode=$1
    local max_wait=${2:-60}
    local waited=0
    ...
    log_info "Waiting for services to start (max ${max_wait}s)..."
    
    while [ $waited -lt $max_wait ]; do
        # Check if key services are running based on mode
        case $mode in
            "simple"|"demo"|"development")
                if check_service_health 8080 "Backend" && check_service_health 3000 "Frontend"; then
                    return 0
                fi
                ;;
            "mock")
                if check_service_health 8000 "Mock" && check_service_health 3000 "Frontend"; then
                    return 0
                fi
                ;;
            "production"|"complete")
...
test_mode() {
    ...
    "$STARTUP_DIR/quick-start.sh" "$mode" --timeout=30 > "$startup_log" 2>&1 &
    local startup_pid=$!
    
    # Wait for startup or timeout
    if wait_for_startup "$mode" "$max_wait"; then
        ...
        # Kill the startup script since services are running
        ...
        # Kill the startup script
        kill $startup_pid 2>/dev/null || true
        
        echo "" >> "$REPORT_FILE"
        echo "**Error Log (last 30 lines):**" >> "$REPORT_FILE"
        echo '```' >> "$REPORT_FILE"
        tail -n 30 "$startup_log" >> "$REPORT_FILE" 2>/dev/null || echo "No log file found" >> "$REPORT_FILE"
        echo '```' >> "$REPORT_FILE"
    fi
...
check_services_for_mode() {
    ...
    for port in 3000 3001 8000 8080 8001 8002 8003 8004; do
        if check_port_occupied $port; then
            local process_info=$(lsof -i :$port 2>/dev/null | tail -n +2 | head -n 1 | awk '{print $1, $2, $9}')
...
get_mode_timeout() {
    local mode=$1
    case $mode in
        "simple"|"demo"|"development"|"mock") echo 90 ;;
        "production"|"complete") echo 150 ;;
        *) echo 90 ;;
    esac
}
...
# Main test execution
main() {
    log_info "Starting OpenPenPal startup modes test (manual version)"
    ...
    echo ""
    echo "Quick summary:"
    grep -E "^### Mode:|^\*\*Status\*\*:" "$REPORT_FILE" | sed 'N;s/\n/ - /'
}
