...
import { validateOPCodeAccess } from '@/lib/utils/courier-permission-utils'
...
export default function CourierOPCodeManagePage() {
  ...
  const filterApplicationsByLevel = (apps: OPCodeApplication[]) => {
    if (!user?.managed_op_code_prefix) return []
    
    return apps.filter(app => {
      const fullCode = app.school_code + app.area_code + '00'
      const permissions = validateOPCodeAccess(
        fullCode,
                  ...
                  <TableBody>
                    {filteredApplications.map((app) => (
                      <TableRow key={app.id}>
                        ...
                        <TableCell>
                          {app.status === 'pending' && courierLevel >= 2 && (
                            <Button
                              size="sm"
                              onClick={() => {
            ...
            <CardHeader>
              ...
              <CardDescription>
                {courierLevel < 2 ? '您没有创建编码的权限' :
                 courierLevel === 2 ? '创建新的投递点编码（后两位）' :
                 courierLevel === 3 ? '创建新的片区或楼栋编码（中间两位）' :
                 '创建新的学校编码（前两位）'}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-4 max-w-md">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={
                        courierLevel === 2 ? '输入完整6位编码' :
                        courierLevel === 3 ? '输入前4位编码' :
                        '输入前2位编码'
                      }
                      maxLength={6}
                    />
