...
func (s *AuditService) ExportAuditLogs(ctx context.Context, filters map[string]interface{}, format string) ([]byte, error) {
	// 查询日志
	entries, _, err := s.QueryAuditLogs(ctx, filters, 1, 10000) // 最多导出10000条
	if err != nil {
		return nil, err
	}
