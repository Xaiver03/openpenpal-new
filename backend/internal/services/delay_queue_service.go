...
// NewDelayQueueService 创建增强版延迟队列服务
...
func (s *DelayQueueService) processDelayedAIReplyTask(task *DelayQueueTask) error {
	...
	_, _ = task.Payload["original_letter"].(string) // 保留以备将来使用
...
func (s *DelayQueueService) handleTaskError(task *DelayQueueTask, err error) {
	// 检查是否为永久错误
...
// 断路器相关方法
func (s *DelayQueueService) isCircuitOpen(key string) bool {
...
func (s *DelayQueueService) recordCircuitFailure(key string) {
	cb, exists := s.circuitBreaker[key]
	if !exists {
...
func (s *DelayQueueService) recordCircuitSuccess(key string) {
	cb, exists := s.circuitBreaker[key]
	if !exists {
		return
	}
	...
	if cb.State == "half_open" {
		...
		cb.FailureCount = delayQueueMaxInt(0, cb.FailureCount-1)
	}
}
...
// GetStats 获取服务统计信息
func (s *DelayQueueService) GetStats() map[string]interface{} {
...
// 辅助函数
func delayQueueMaxInt(a, b int) int {
	if a > b {
		return a
	}
	return b
