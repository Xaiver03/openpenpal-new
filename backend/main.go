...
func main() {
	...
	if os.Getenv("SKIP_DB_MIGRATION") == "true" {
		log.Info("SKIP_DB_MIGRATION is set - using database without migration")
		db, err = config.SetupDatabaseWithoutMigration(cfg)
	} else {
	...
	// 初始化缓存管理器（用于中间件和服务）
	var enhancedCacheManager *cache.EnhancedCacheManager
	if redisClient != nil {
		enhancedCacheManager = cache.NewEnhancedCacheManager(
			redisClient,
			5*time.Minute, // Default TTL
			"openpenpal",  // Key prefix
		)
		// 设置jitter为15%防止缓存雪崩
		enhancedCacheManager.SetJitterPercent(15)
		log.Info("Enhanced cache manager initialized successfully with anti-avalanche protection")
	}
	...
	driftBottleService := services.NewDriftBottleService(db, letterService, notificationService) // 漂流瓶服务
	futureLetterSchedulerService := services.NewFutureLetterService(db, letterService, notificationService) // 未来信调度服务
	...
	*/
	...
	// 新增：整合后的信使处理器
	courierDashboardHandler := handlers.NewCourierDashboardHandler(db, courierService, courierTaskService, analyticsService)
	courierProfileHandler := handlers.NewCourierProfileHandler(db, courierService, promotionService, creditService)
	...
	unifiedShopHandler := handlers.NewUnifiedShopHandler(db, shopService, creditService, creditShopService) // 统一商城处理器
	...
	// 初始化完整性和审计服务 - 暂时禁用
	// Initialize integrity and audit services
	...
	driftBottleHandler := handlers.NewDriftBottleHandler(driftBottleService)           // 漂流瓶处理器
	futureLetterHandler := handlers.NewFutureLetterHandler(futureLetterSchedulerService) // 未来信处理器
	...
	*/
	...
	// 全局中间件 - SOTA级别配置
	// 1. 基础中间件
	// 增强请求追踪中间件（替换基础RequestIDMiddleware）
	tracingConfig := middleware.TracingConfig{
		ServiceName:     "openpenpal-backend",
		EnableLogging:   true,
		EnableMetrics:   true,
		LogRequestBody:  cfg.Environment == "development", // 只在开发环境记录请求体
		LogResponseBody: false,                            // 响应体通常较大，避免记录
		MaxBodySize:     1024 * 10,                        // 10KB最大记录大小
	}
	router.Use(middleware.RequestTracingMiddleware(tracingConfig))
	...
	// 1.1 幂等性中间件（用于POST/PUT/PATCH请求）
	if enhancedCacheManager != nil {
		idempotencyConfig := middleware.IdempotencyConfig{
			CacheManager:   enhancedCacheManager,
			TTL:            24 * time.Hour,                                  // 24小时幂等保护
			SkipPaths:      []string{"/health", "/ping", "/metrics", "/ws"}, // 跳过这些路径
			AllowedMethods: []string{"POST", "PUT", "PATCH"},                // 只对这些方法启用
		}
		router.Use(middleware.IdempotencyMiddleware(idempotencyConfig))
		log.Info("Idempotency middleware enabled with 24h TTL")
	} else {
		log.Warn("Idempotency middleware disabled - cache manager not available")
	}
	...
	public := v1.Group("/")
	{
		...
		{
			courier.GET("/stats/global", courierHandler.GetCourierStats) // 全局统计信息
		}
		...
		// 公开的AI相关（无需认证）- 已注释掉避免重复路由
		// ai := public.Group("/ai")
		// {
		//	ai.POST("/match", aiHandler.MatchPenPal)
		//	ai.POST("/reply", aiHandler.GenerateReply)
		//	ai.POST("/reply-advice", aiHandler.GenerateReplyAdvice)
		...
		*/
		...
		{
			publicUsers.GET("/:username/profile", userProfileHandler.GetUserProfile)     // 获取用户公开资料
			publicUsers.GET("/:username/letters", userProfileHandler.GetUserLetters)     // 获取用户公开信件
		}
		
		// 关注状态查询（使用不同的路径避免冲突）
		public.GET("/follow/status/:id", followHandler.GetFollowStatus)               // 获取关注状态
	...
	protected.Use(middleware.AuthMiddleware(cfg, db))
	{
		...
		// 漂流瓶相关
		driftBottleHandler.RegisterDriftBottleRoutes(protected)

		// 未来信相关
		futureLetterHandler.RegisterFutureLetterRoutes(protected)
		...
		courier := protected.Group("/courier")
		{
			...
			// 个人信使统计信息（需要认证）
			courier.GET("/stats", courierHandler.GetMyStats) // 个人信使统计
			...
			{
				tasks.GET("", courierHandler.GetCourierTasks)
				tasks.GET("/:id", courierHandler.GetTaskDetail)
				tasks.PUT("/:id/status", courierHandler.UpdateTaskStatus)
				tasks.POST("/scan", courierHandler.ScanCode) // 扫码作为任务的子功能
			}
			...
			// === 保留原有路由以保持向后兼容（标记为deprecated）===
			courier.POST("/apply", courierHandler.ApplyCourier)
			courier.GET("/status", courierHandler.GetCourierStatus)
			courier.GET("/profile", courierHandler.GetCourierProfile) // deprecated -> use /profile/info
			...
			courier.GET("/hierarchy", courierHandler.GetHierarchyInfo)      // 获取层级信息
			...
			// courier.GET("/tasks", courierHandler.GetCourierTasks)        // deprecated - removed duplicate, use /tasks subgroup
			...
			*/
			...
			// 批量管理API - 桥接到信使服务(8002端口)
			batch := courier.Group("/batch")
			{
				// 信号编码批量生成 - 代理到 /api/signal-codes/batch
				batch.POST("/generate", middleware.ProxyToCourierService("/api/signal-codes/batch"))

				// 信号编码分配 - 代理到 /api/signal-codes/assign
				batch.POST("/assign", middleware.ProxyToCourierService("/api/signal-codes/assign"))

				// 信号编码搜索 - 代理到 /api/signal-codes/search
				batch.GET("/search", middleware.ProxyToCourierService("/api/signal-codes/search"))

				// 信号编码统计 - 代理到 /api/signal-codes/stats/:school_id
				batch.GET("/stats/:school_id", middleware.ProxyToCourierService("/api/signal-codes/stats/:school_id"))

				// 信号编码使用日志 - 代理到 /api/signal-codes/:code/logs
				batch.GET("/logs/:code", middleware.ProxyToCourierService("/api/signal-codes/:code/logs"))

				// 信号编码释放 - 代理到 /api/signal-codes/:code/release
				batch.POST("/release/:code", middleware.ProxyToCourierService("/api/signal-codes/:code/release"))
			}

			// OP Code管理 - 信使专用接口
			opcodeManage := courier.Group("/opcode")
			{
				opcodeManage.GET("/applications", courierOPCodeHandler.GetApplications)      // 获取待审核申请
				opcodeManage.GET("/managed", courierOPCodeHandler.GetManagedOPCodes)        // 获取管理的OP Code
				opcodeManage.POST("/create", courierOPCodeHandler.CreateOPCode)             // 创建新OP Code
				opcodeManage.POST("/applications/:id/review", courierOPCodeHandler.ReviewApplication) // 审核申请
			}
		}
		...
		shopAuth := protected.Group("/shop")
		{
			// === 新统一商城API ===
			// 商品浏览（支持现金和积分商品）
			// shopAuth.GET("/products", unifiedShopHandler.GetProducts) // 获取商品列表（统一接口）- 需要认证的商品列表（包含用户特定信息）- 与公开路由冲突
			
			// 购物车相关（统一接口）
			shopAuth.GET("/cart", unifiedShopHandler.GetCart)                         // 获取购物车
			shopAuth.POST("/cart/add", unifiedShopHandler.AddToCart)                  // 添加到购物车
			shopAuth.PUT("/cart/items/:id", unifiedShopHandler.UpdateCartItem)        // 更新购物车项目
			shopAuth.DELETE("/cart/items/:id", unifiedShopHandler.RemoveCartItem)     // 删除购物车项目
			shopAuth.DELETE("/cart", unifiedShopHandler.ClearCart)                    // 清空购物车
			
			// === 保留原有API以兼容 ===
			// 商品评价（需要认证）
			shopAuth.POST("/products/:id/reviews", shopHandler.CreateProductReview) // 创建商品评价

			// 旧购物车API（标记为deprecated）
			shopAuth.POST("/cart/items", shopHandler.AddToCart)            // 添加商品到购物车 [deprecated]
			// shopAuth.DELETE("/cart/items/:item_id", shopHandler.RemoveFromCart) // 从购物车移除商品 [deprecated] - conflicts with :id parameter
		...
		opcode := protected.Group("/opcode")
		{
			...
			// 注意：不需要 ValidateOPCodeWithAuth，因为公开版本的 ValidateOPCode 已经能处理认证情况
			...
			// 注意：不需要 GetOPCodeWithAuth，因为公开版本的 GetOPCode 已经能智能处理认证情况
	...
	})
	{
		...
		// AI管理路由已在routes.SetupAIRoutes中定义，避免重复
